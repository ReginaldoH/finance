...
<div class="page-content header-clear-medium" style="margin-top: -25px">
  ...
  <!-- Label flutuante -->
  <label class="color-theme ps-5"></label>
</div>
...
$nome = $nome_cliente;
  ...
  <strong><big>{$nome_cliente}</big></strong><br>  
  <span>Parcelas: <span style="color:blue">{$parcelas_nome}</span> <span style="color:red">{$atrasadas}</span></span><br>
  <span>Gerada Em: {$dataF}</span><br> 
  <span>Frequência: <strong>{$frequencia}</strong></span><br> 
  <span><small>Próxima Parcela: {$data_ultimo_vencF}</small></span><br>
</div>
      ...
      <form method="POST" action="../../painel/rel/detalhamento_cobranca_class.php"  style="display:inline;">
        <input type="hidden" name="id" value="{$id_cob}">
        <button type="submit" class="icon icon-xs rounded-circle shadow-l" style="background: red; color:#FFF; outline:none; border:none" title="Detalhamento Empréstimo">
          <i class="fa fa-file-pdf" style="font-size:14px;"></i>
                ...
                <a href="#" onclick="mostrarParcelasEmp('{$id_cob}')" class="icon icon-xs rounded-circle shadow-l" style="background: #258a56; color:#FFF;" title="Mostrar Parcelas">
