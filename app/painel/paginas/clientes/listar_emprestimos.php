...
for($i=0; $i < $total_reg; $i++){
	foreach ($res[$i] as $key => $value){}
		...
		$contatoF = '';
		if ( strlen((string)$contato) > 0 ) {
			$contatoF = "<br><b>".$contato."</b>";
		}
		$obs = $res[$i]['obs'];
		$obsF = '';
		if ( strlen((string)$contato) > 0 ) {
	...
	$res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
	$total_nao_pago = @count($res2);
	$receber_pago = $parcelas - $total_nao_pago;
	...
	echo <<<HTML
	<tr>					
		<td  style="line-height: 1.5;" class="{$classe_deb}">R$ {$valorF} - ({$receber_pago}/{$parcelas}) {$classe_finalizado}
		{$contatoF} {$obsF}
		</td>				
		<td style="padding: 0.5rem 0.0rem !important;">
