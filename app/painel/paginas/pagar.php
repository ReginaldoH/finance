  ...
  <div class="card card-style">
    <div class="content">
      ...
      if ($pago == 'Vencidas') {
        ...
        $query = $pdo->query("SELECT * from $pag where data_venc >= '$dataInicial' and data_venc <= '$dataFinal' and pago LIKE '%$pago%' order by id desc LIMIT $limite, $itens_pag");
      }
      $valor_pago = 0;
      $valor_pendentes = 0;
  ...
  <div class="splide__track">
    <div class="splide__list">
      <div class="splide__slide mx-3">
        <div class="d-flex">
          <div
            onclick="mostrar('{$descricao}','{$valor_finalF}','{$data_vencF}','{$data_pgtoF}','{$nome_usuario_lanc}','{$nome_usuario_baixa}','{$pago}','{$referencia}','{$obs}')">
      ...
      <div class="splide__slide mx-3">
        <div class="d-flex">
          ...
          <div class="ms-auto">
            <a onclick="editar('{$id}','{$descricao}','{$valor}','{$data_venc}','{$obs}')" href="#" class="{$ocultar} icon icon-xs rounded-circle shadow-l bg-twitter"><i class="fa fa-edit text-white"></i></a>
...
<script type="text/javascript">
  function mostrar(descricao, valor, data_venc, data_pgto, nome_usuario_lanc, nome_usuario_baixa, pago, referencia, obs) {
...
function baixarConta(id) {
    ...
    swalWithBootstrapButtons.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: '../../painel/paginas/pagar/baixar.php',
                method: 'POST',
                data: { id },
                dataType: "html",
